...
largura_min=100 # 최소 직사각형 너비
altura_min=100 # 직사각형의 최소 높이

offset=10 # 픽셀 간 허용 오차

pos_linha=150 # 카운트 라인 위치
linw = 10 # 카운트 라인 두께
...
# 원본 동영상 크기 정보
w = cap.get(cv2.CAP_PROP_FRAME_WIDTH) # 640
h = cap.get(cv2.CAP_PROP_FRAME_HEIGHT) # 480
...
while True:
    ...
    cv2.line(frame1, (pos_linha, 0), (pos_linha, 480), (255,127,0), linw)  # 카운트 라인 그리기 (이미지 파일, 시작점 좌표 (x, y), 종료점 좌표 (x, y), 색상 (blue, green, red), 선 두께, 선 종류, fractional bit)
    for(i,c) in enumerate(contorno): # 카운트 라인에 닿았을 때
        ...
        detec.append(centro)
        cv2.circle(frame1, centro, 4, (0, 0,255), -1) # 원 그리기a

        for (x,y) in detec:
            if x<(pos_linha+offset) and x>(pos_linha-offset): # 카운트 라인 안에 들어올 때
                carros+=1 # 카운트 값 증가
                cv2.line(frame1, (pos_linha, 0), (pos_linha, 480), (0,127,255), linw) # 선 색상 변경
                detec.remove((x,y)) # 초기화
                print("Person is detected : "+str(carros)) # 카운트 출력
       
    cv2.putText(frame1, "Person COUNT : "+str(carros), (0, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2) # 텍스트 출력, (이미지 파일, 출력 문자, 출력 문자 시작 위치 좌표 (좌측 하단), fontFace,  폰트 크기, 폰트 색상, 폰트 두께, 선 종류, org 사용 옵션 (True: 좌측 하단, False: 좌측 상단))
